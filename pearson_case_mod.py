...
def pearson_case_mod(train, rating,movies, avg_train_r,avg_r):
	sims=[]
	for i in range(len(train)):
		...
		for x in range(len(rating)):
			if int(train[i][int(rating[x])-1])==0:
				continue
			numerater+= (int(rating[x])-avg_r) * (int(train[i][int(movies[x])-1])-avg_train_r[i])
			denoa+=(int(rating[x])-avg_r)**(2)
			denob+=(int(train[i][int(rating[x])-1])-avg_train_r[i])**(2)
			counter+=1
		if denob==0.0 or denoa==0.0 or counter==1:
			sims.append(0)
		else:
			sims.append(numerater/(math.sqrt(denoa)*math.sqrt(denob)))
	return sims
...
def tester(input, output, test_num):
	...
	for x in range(len(test)):
		...
		if int(test[x][2]) != 0:
			...
			counter+=1
		if len(test_r)==test_num:
			sim_list.append(pearson_case_mod(train,test_r,movies,avg_train_r,avg_test_r[avg_count]))
			test_r.clear()
			counter=0
			avg_count+=1
